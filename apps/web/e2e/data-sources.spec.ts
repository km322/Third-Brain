...
test.describe("data sources", () => {
  /**
   * The sync reads a folder on disk the API server can read. When the stack runs in
   * containers (CI), E2E_DATA_DIR is a host path bind-mounted into api + worker at the
   * same absolute path; otherwise the API shares the host filesystem and tmp is fine.
   * mkdtemp creates the dir 0700 - it is widened so the container user (uid 10001) can
   * traverse and read the tree. The folder's `.acl.json` carries an empty default ACL, so
   * only admins (the owner) can see it - which is who we are.
   *
   * In the dialog, kind defaults to "Local folder", so only the target collection is
   * picked. The folder has exactly one file, and that synced file is ingested through the
   * normal pipeline and lands Indexed.
   */
