...
async def get_file(
    ...
    Uniform 404s: a bad token, a non-image document, and a quarantined document are
    indistinguishable, so the endpoint leaks nothing about what exists.
    """
    if not (_MIN_TOKEN_LEN <= len(token) <= _MAX_TOKEN_LEN):
        raise HTTPException(status_code=404, detail="Not found")
    # Consult (without counting) the miss budget BEFORE the DB probe: once a scanner is
    # throttled it must stop driving lookups entirely, not just receive a different
    # status code. Only actual misses increment the counter, below.
    ...
    if (
        ...
        # Throttle only MISSES, keyed per client (not per token - a per-token bucket would
        # give every guess a fresh budget and never throttle a scan).
        await enforce_login_rate_limit(request, "files")
        raise HTTPException(status_code=404, detail="Not found")

    try:
        data = await get_storage().load(document.storage_key)
    except Exception as exc:  # blob missing/unreadable - treat as gone
        raise HTTPException(status_code=404, detail="Not found") from exc

    # Authorize on the BYTES, never on the client-supplied mime_type recorded at upload.
    # Trusting that field would turn this public endpoint into an unauthenticated server
    # for arbitrary unscanned content (upload any file labelled image/png).
