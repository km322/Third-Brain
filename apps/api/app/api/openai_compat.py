...
# --------------------------------------------------------------------------- #
def _load_rag() -> Any | None:
    """Lazily import the RAG service (owned by the search module). ``None`` if absent."""
    try:
        from app.services import rag  # type: ignore
    except Exception:  # pragma: no cover - module may not exist during build
        return None
    return rag


async def _call_rag_answer(
    rag: Any,
    db: AsyncSession,
    ctx: AuthContext,
    query: str,
    ...
    """Call ``rag.answer`` tolerantly across minor signature variations."""
    kwargs: dict[str, Any] = {"top_k": top_k, "temperature": temperature}
    ...
    try:
        return await rag.answer(db, ctx, query, **kwargs)
    except TypeError:
        return await rag.answer(db, ctx, query)
...
def _normalize_citation(citation: Any) -> dict[str, Any]:
    get = (
        ...
        else (lambda key, default=None: getattr(citation, key, default))
    )
    return {
        "document_id": str(get("document_id", "") or ""),
        "document_title": get("document_title", "") or "",
        "collection_id": str(get("collection_id", "") or ""),
...
def _adapt_rag_result(result: Any) -> tuple[str | None, list[dict], dict]:
    """Extract (text, citations, usage) from whatever shape ``rag.answer`` returned.

    ``app.services.rag.answer`` returns a ``(answer_text, cited_hits)`` tuple and records
    its own COMPLETION usage, so ``usage`` is empty for that shape - the caller must not
    record it again (that was double-billing every request and re-running the pipeline).
    """
    if result is None:
        return None, [], {}
    if isinstance(result, tuple):
        # ``rag.answer`` returns ``(answer_text, cited_hits)`` where hits are SearchHits.
        text = result[0] if len(result) > 0 else None
        hits = result[1] if len(result) > 1 else []
        citations = [
            {
    ...
    return text, [_normalize_citation(c) for c in citations], usage
...
async def _inline_ground(
    ...
    """Self-contained RAG used when ``app.services.rag`` is unavailable.
    ...
    return result.text, citations, usage
...
async def _grounded(
    ...
    from ``rag.answer`` - which records its own EMBEDDING + COMPLETION usage - so the caller
    must NOT record usage again; it is False for the inline fallback, which the caller meters.
    """
    rag = _load_rag()
    if rag is not None and hasattr(rag, "answer"):
        try:
            result = await _call_rag_answer(
                rag, db, ctx, query, collection_ids, top_k, model, temperature, max_tokens
            )
            text, citations, usage = _adapt_rag_result(result)
            if text is not None:
                return text, citations, usage, True
