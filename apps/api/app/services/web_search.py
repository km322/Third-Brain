...
in dev/CI so web grounding is fully testable, while a real provider (Tavily/Bing/Brave)
can be dropped in with an API key. ``none`` (the default) disables web grounding entirely.
...
def web_search_enabled() -> bool:
    return settings.WEB_SEARCH_PROVIDER not in ("", "none")
