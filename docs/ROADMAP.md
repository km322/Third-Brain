...
Third Brain is free and Apache-2.0: everything listed as shipped is in this repository, with no
tiers and nothing held back for a hosted version. Items tagged *(contributions welcome)* are
self-contained enough to pick up - see [`CONTRIBUTING.md`](../CONTRIBUTING.md).
...
The baseline the roadmap builds on - all of it in the repo and covered by tests:

- **Agentic write-back (the documentation writes itself)** - agents over MCP call `add_knowledge`
  / `update_knowledge` to capture decisions, answers and notes as they work. Writes pass through
  the same secret/DLP quarantine and permission model as everything else, so an agent only writes
  where it's allowed and never persists a leaked credential.
...
- **Multi-provider LLM layer** - BYO-keys connectors for any OpenAI-compatible endpoint (OpenAI,
  ...
  Gemini**, over plain `httpx` with no provider SDKs, and a deterministic offline stub so the
  stack runs with zero keys.
- **Three ways in** - native REST under `/api/v1`, an OpenAI-compatible `/v1` endpoint, and a
...
- **Agent-written tracking** - the dashboard Documents page carries an **Agent-written** filter and
  badge, and the Overview a **Written by agents** card, so a team sees exactly what its agents
  captured.
- **Ingestion** - text, URL and file upload, chunked and embedded by arq workers into Postgres +
  `pgvector`. Raster images are described and transcribed by a vision model, then indexed as text
  carrying a capability link back to the original bytes.
- **Data sources** - external sync that mirrors a source system's ACLs into Third Brain grants, so
  ...
  Google Drive, Slack, GitHub, Notion and Confluence validate and store configuration, but their
  live fetch is not built yet.
...
- **SSO / SAML + SCIM** - OIDC and SAML 2.0 sign-in with just-in-time provisioning (SAML assertions
  are signature-verified against the connection's IdP certificate), plus SCIM 2.0 provisioning of
  users and of IdP groups as teams. Both are set up from the dashboard's **SSO & SCIM** page.
- **Sensitivity classification + oversharing report** - a DLP scan labels ingested content `pii` /
  `confidential` (`DLP_ENABLED`, on by default; `DLP_DEFAULT_ACTION` picks label, quarantine or
  warn), and an admin-only report lists sensitive documents that are nonetheless org- or
  public-visible.
- **Verification & freshness** - documents and curated answers can be marked verified with a
  review-by date (interval defaults to `DEFAULT_REVIEW_INTERVAL_DAYS`); an hourly worker flips
...
- **Evaluation & load-test harnesses** - a golden-dataset retrieval + permission-correctness
  ...
  [`BENCHMARKING.md`](./BENCHMARKING.md)) and a Locust load test with pass/fail latency and error
  thresholds (`make load-test` / `make load-seed`, see [`LOAD_TESTING.md`](./LOAD_TESTING.md)).
  They run on demand today; wiring them into CI as release gates is near-term.
...
Harden what exists, then unlock the two things that gate retrieval quality: live connectors and
better ranking.

- **Harden for production.** Tighter rate-limit and quota enforcement, more coverage on the
  permission boundary, structured audit events for every write, background-job retries and
  dead-lettering, and running the load/soak harness on a schedule.
- **Finish SCIM group sync.** SSO and SCIM ship today, but there is no
  `PATCH /scim/v2/Groups/{id}` - the call an IdP makes to push membership changes after a group
  exists - so a team only tracks its IdP group as of creation time.
- **Live data-source connectors - Slack, Google Drive, Notion.** Incremental sync that ingests from
  source systems and, critically, **maps source-side permissions onto Third Brain ACLs** so the
  permission gate holds end-to-end. The sync engine, source-ACL mapping and identity resolution
  already ship behind the reference `local_folder` connector (see **Data sources** in
...
- **Gate releases on the eval harness.** The retrieval-quality *and* permission-correctness eval
  already exists (`make benchmark`); the work is wiring it into CI so every release is blocked on
  it - the permission suite is non-negotiable, since one leaked chunk destroys trust - and
  publishing the permission-correctness benchmarks.
- **Email verification and self-serve password reset**, so members can recover their own accounts
  instead of asking an admin; today only an admin can reset a member's password. The delivery
  pipeline underneath already ships: org invites send a tokenised acceptance link through the
  pluggable `EMAIL_PROVIDER`, which defaults to a `stub` that captures mail instead of sending it,
  so a deployment that wants invites delivered sets `EMAIL_PROVIDER=smtp` (see
  [`DEPLOYMENT.md`](./DEPLOYMENT.md)).
- **Runtime web configuration.** Move the web client's API base URL from the build-time
  `NEXT_PUBLIC_API_URL` to server-provided runtime config, so one published web image serves any
  deployment instead of being baked per-origin. *(Contributions welcome - it would let every
...
- **Reviewable agent write-back.** Agents already write back today, gated by permissions and the
  secret scanner; this layer adds suggested edits, richer provenance and human-in-the-loop approval
  queues so higher-stakes contributions can be reviewed before they land.
...
- **Client SDKs.** First-class Python and TypeScript SDKs with typed clients, streaming and MCP
  helpers. The CLI shipped first as the fastest way to wire agents in; typed SDKs are the next
  layer for teams building directly against the REST and OpenAI-compatible surfaces.
...
- **Cross-org knowledge federation.** Opt-in, permission-preserving sharing between organizations
  (a company and its vendors, say) without collapsing either side's ACLs.
...
1. **Permission correctness first.** Anything touching retrieval ships behind the eval harness; the
   permission gate never regresses.
2. **Then whatever a self-hoster is blocked on.** Setup, upgrade and operational papercuts beat new
   surface area: the project is only useful if a stranger can stand it up and keep it running.
...
Every part of this is fair game. Especially welcome:
...
- **Retrieval evaluation.** New golden-dataset cases for `make benchmark`, particularly adversarial
  permission cases: a case that catches a leak is the most valuable contribution here.
