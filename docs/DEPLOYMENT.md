...
| `MAX_REQUEST_BODY_BYTES` | Hard ceiling on a single request body (default 32 MiB), enforced before a handler buffers it. Keep it above the 25 MiB document-upload cap. |

Features that ship **off** and stay inert until you configure them. Each still has a visible
surface in the product, so decide deliberately rather than hearing about it from a user:

| Variable | Default | What is off until you set it |
|---|---|---|
| `EMAIL_PROVIDER` | `stub` | Transactional email. Invites are the only flow that sends mail, and their accept link is returned by no API and shown nowhere in the dashboard, so on the default an invite **goes nowhere**. `console` logs only the recipient and subject (the body carries the accept token and is deliberately never logged), so `smtp` + the `SMTP_*` settings is the only configuration that delivers one. |
| `WEB_SEARCH_PROVIDER` | `none` | Web grounding for the assistant. The dashboard's Ask page shows a **Web** switch either way; while this is `none` the switch grounds nothing. `stub` returns deterministic offline results; `tavily` is the only live provider implemented and needs `WEB_SEARCH_API_KEY` (`WEB_SEARCH_BASE_URL` overrides the API host, `WEB_SEARCH_MAX_RESULTS` the result count, default 3). Web hits are external citations and never widen document access. |
| `LOCAL_CONNECTOR_ROOTS` | empty | The reference `local_folder` data-source connector, which reads files off the server's own disk. Empty disables it; list the absolute paths org admins may ingest from. |
...
- [ ] `EMAIL_PROVIDER=smtp` + `SMTP_*` set if you intend to invite teammates - on the default
      `stub` the invite link is never delivered and never displayed.
