...
(default `none`; `stub` gives deterministic offline results, and `tavily` - the only live
provider implemented - needs `WEB_SEARCH_API_KEY`), and web results never widen document
access.
