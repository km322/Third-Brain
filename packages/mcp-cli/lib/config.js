...
export function saveConfig(config, configPath = CONFIG_PATH) {
  ...
  // writeFile only applies the mode on creation; re-assert it for existing files.
...
export function resolveConfig(env = process.env, configPath = CONFIG_PATH) {
  ...
  // Normalize here so every consumer (serve, status) gets the same canonical URL - a bare
  // host in THIRD_BRAIN_URL works everywhere, not just where the caller happened to
  // normalize. Fall back to the raw value if it cannot be parsed; downstream reports it.
  ...
  if (rawUrl) {
    try {
      ...
      // Never silently downgrade to cleartext: a refused insecure-http URL must surface, not
      // fall through as a raw value the key would then be sent over. A plain parse error keeps
      // the raw value so a caller (status/serve) can still report "could not reach ...".
